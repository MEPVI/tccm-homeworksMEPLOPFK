...
int main() {
   ...
   double energy; 
    ...
    // declare variable for storing nuc energy
     //double  energy=0.0;
   trexio_exit_code trexio_read_nucleus_repulsion(trexio_file, &energy);
...
// Declare variable to store the number of spin-up electrons
   ...
   trexio_exit_code read_electron_up_num(file, &n_up);

    int32_t mo_num;
    double* mo_energy;
  trexio_exit_code read_mo_data(file, &mo_num, &mo_energy);
// occupied number of orbitals
   ...
   // int32_t saved_mo_num = mo_num;
    ...
    int32_t* index;
    double* value;
    trexio_exit_code read_two_electron_integrals(file, &n_integrals, &index, &value);
